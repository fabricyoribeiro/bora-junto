...
export default function Agenda({ onLogout }) {
  ...
  async function fetchEvents(date) {
    try {
      ...
      const response = await api.get(`/event/date/${date}`, {user_id:userId});
      const data = response.data;
      setEvents(data);
      console.log(data);
    } catch (error) {
      ...
      <View style={styles.container}>
        ...
        <ScrollView style={{ flex: 1, flexDirection: 'column', marginBottom: 100 }}>
          ...
          {Array.isArray(newEventForms) &&
            newEventForms.map((e, index) => <EventForm key={e.id} addNewForm={addNewEventForm} eventDate={value} fetchEvents={fetchEvents} />)}
        </ScrollView>
      </View>
    </SafeAreaView>
  );
